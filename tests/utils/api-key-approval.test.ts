...
describe('aPI Key Approval Management', () => {
  const CCR_API_KEY = 'sk-tkp-x-ccr'
  ...
  describe('edge Cases', () => {
    ...
    it('should truncate API key to 20 characters for storage', () => {
      ...
      const longApiKey = 'sk-tkp-x-ccr-very-long-api-key-that-exceeds-20-characters'
      const expectedTruncated = longApiKey.substring(0, 20) // 'sk-tkp-x-ccr-very-lo'
    ...
    it('should handle exactly 20 character API key without truncation', () => {
      ...
      const exactApiKey = 'sk-tkp-x-ccr-exact20'
