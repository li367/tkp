...
describe('installation manager utilities', () => {
  beforeEach(() => {
    ...
    vi.mocked(tkpConfig.getTkpConfig).mockReturnValue({
      version: '1.0.0',
      preferredLang: 'en',
  ...
  describe('handleMultipleInstallations', () => {
    ...
    it('should save local installation path to TKP config when user chooses local', async () => {
      const installStatus = {
        hasGlobal: true,
        hasLocal: true,
        localPath: '/Users/test/.claude/local/claude',
      ...
      expect(tkpConfig.updateTomlConfig).toHaveBeenCalledWith('/Users/test/.tkp/config.toml', {
        claudeCode: {
          installType: 'local',
        },
      })
    ...
    it('should save global installation info to TKP config when user chooses global', async () => {
      const installStatus = {
        hasGlobal: true,
        hasLocal: true,
        localPath: '/Users/test/.claude/local/claude',
    ...
    it('should respect previous user choice when both installations exist', async () => {
      ...
      vi.mocked(tkpConfig.readTomlConfig).mockReturnValue({
        version: '1.0.0',
        lastUpdated: '2025-01-01T00:00:00.000Z',
        general: {
          preferredLang: 'en',
    ...
    it('should handle config update failure gracefully', async () => {
      ...
      vi.mocked(inquirer.prompt).mockResolvedValue({ installMethod: 'local' })
      vi.mocked(tkpConfig.updateTomlConfig).mockImplementation(() => {
      ...
      expect(tkpConfig.updateTomlConfig).toHaveBeenCalled()
  ...
  describe('getClaudeCodeConfigDir', () => {
    ...
    it('should return default config dir when getTkpConfig throws error', () => {
      vi.mocked(tkpConfig.getTkpConfig).mockImplementation(() => {
  ...
  describe('handleMultipleInstallations - additional edge cases', () => {
    ...
    it('should not call updateTomlConfig on error scenarios', async () => {
      ...
      expect(tkpConfig.updateTomlConfig).not.toHaveBeenCalled()
    })
  })
