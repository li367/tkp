...
describe('menu command', () => {
  ...
  describe('showMainMenu', () => {
    it('should display menu and handle exit', async () => {
      ...
      vi.mocked(readTkpConfig).mockReturnValue({ preferredLang: 'zh-CN', codeToolType: 'claude-code' } as any)
    ...
    it('should display banner with Claude Code label', async () => {
      ...
      vi.mocked(readTkpConfig).mockReturnValue({ preferredLang: 'en', codeToolType: 'claude-code' } as any)
      vi.mocked(inquirer.prompt).mockResolvedValue({ choice: 'q' })
    ...
    it('should display banner with Codex label', async () => {
      ...
      vi.mocked(readTkpConfig).mockReturnValue({ preferredLang: 'en', codeToolType: 'codex' } as any)
      vi.mocked(inquirer.prompt).mockResolvedValue({ choice: 'q' })
    ...
    it('should handle language change option', async () => {
      ...
      vi.mocked(readTkpConfigAsync).mockResolvedValue({ preferredLang: 'zh-CN', codeToolType: 'claude-code' } as any)
      vi.mocked(inquirer.prompt)
    ...
    it('should handle CCU usage analysis option in English', async () => {
      ...
      vi.mocked(readTkpConfigAsync).mockResolvedValue({ preferredLang: 'en', codeToolType: 'claude-code' } as any)
      vi.mocked(inquirer.prompt)
        .mockResolvedValueOnce({ choice: 'u' })
      queuePromptBooleans(false)
      vi.mocked(runCcusageFeature).mockResolvedValue(undefined)
    ...
    it('should allow switching code tool type', async () => {
      ...
      expect(updateTkpConfig).toHaveBeenCalledWith(expect.objectContaining({ codeToolType: 'codex' }))
    })
  ...
  describe('menu edge cases through public API', () => {
    ...
    it('should handle cancelled tool selection', async () => {
      ...
      expect(updateTkpConfig).not.toHaveBeenCalledWith(expect.objectContaining({ codeToolType: expect.anything() }))
    })
    ...
    it('should handle same tool selection', async () => {
      ...
      expect(updateTkpConfig).not.toHaveBeenCalledWith(expect.objectContaining({ codeToolType: 'claude-code' }))
    })
  ...
  describe('showMainMenu with codeType parameter', () => {
    it('should switch code tool type when valid codeType parameter is provided', async () => {
      ...
      // Verify updateTkpConfig was called to switch tool type
    ...
    it('should not update config when codeType matches current tool', async () => {
      ...
      // Verify updateTkpConfig was NOT called since type matches
