...
const tkpConfigMock = vi.hoisted(() => ({
  readTkpConfig: vi.fn(() => ({ codeToolType: 'claude-code' })),
  readTkpConfigAsync: vi.fn(async () => ({ codeToolType: 'claude-code' })),
}))

const resolveCodeTypeMock = vi.hoisted(() => vi.fn(async () => 'claude-code'))

vi.mock('../../src/utils/tkp-config', () => ({
  readTkpConfig: tkpConfigMock.readTkpConfig,
  readTkpConfigAsync: tkpConfigMock.readTkpConfigAsync,
}))
...
describe('uninstall command', () => {
  ...
  describe('interactive mode', () => {
    ...
    it('should show custom uninstall options when custom is selected', async () => {
      ...
      expect(secondPrompt.choices).toEqual(expect.arrayContaining([
        ...
        expect.objectContaining({ value: 'tkp-config' }),
      ]))
  ...
  it('should fall back to config when code type resolution fails and run Codex uninstaller', async () => {
    ...
    tkpConfigMock.readTkpConfig.mockReturnValueOnce({ codeToolType: 'codex' })
