...
describe('withLanguageResolution', () => {
  let mockAction: Mock
  let mockReadTkpConfigAsync: Mock
  ...
  beforeEach(async () => {
    ...
    const tkpConfigModule = await import('../../src/utils/tkp-config')
    const promptsModule = await import('../../src/utils/prompts')
    const i18nModule = await import('../../src/i18n')

    mockReadTkpConfigAsync = vi.mocked(tkpConfigModule.readTkpConfigAsync)
  ...
  describe('skipPrompt handling', () => {
    it('should respect skipPrompt from wrapper parameter', async () => {
      // Arrange
      mockReadTkpConfigAsync.mockResolvedValue({})
  ...
  describe('edge cases', () => {
    it('should handle config read errors', async () => {
      // Arrange
      mockReadTkpConfigAsync.mockRejectedValue(new Error('Config read failed'))
      const wrappedAction = await withLanguageResolution(mockAction, true)
