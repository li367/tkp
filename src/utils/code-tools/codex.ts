...
import { AI_OUTPUT_LANGUAGES, CODEX_AGENTS_FILE, CODEX_AUTH_FILE, CODEX_CONFIG_FILE, CODEX_DIR, CODEX_PROMPTS_DIR, SUPPORTED_LANGS, TKP_CONFIG_FILE } from '../../constants'
...
import { readDefaultTomlConfig, readTkpConfig, updateTkpConfig, updateTomlConfig } from '../tkp-config'
...
export interface CodexMcpService {
  ...
  // Preserve all extra configuration fields not directly managed by TKP
...
export interface CodexConfigData {
  ...
  managed: boolean
  otherConfig?: string[] // Lines that are not managed by TKP
...
export function backupCodexFiles(): string | null {
  ...
  if (process.env.TKP_CODEX_SKIP_PROMPT_SINGLE_BACKUP === 'true' && cachedSkipPromptBackup)
  ...
  if (process.env.TKP_CODEX_SKIP_PROMPT_SINGLE_BACKUP === 'true')
...
export function migrateEnvKeyToTempEnvKey(): boolean {
  ...
  try {
    ...
    // Update TKP config to mark migration as complete
    updateTomlConfig(TKP_CONFIG_FILE, {
      codex: {
        envKeyMigrated: true,
      },
    } as any)
...
export function ensureEnvKeyMigration(): void {
  // Check TKP config to see if migration has already been done
...
export function parseCodexConfig(content: string): CodexConfigData {
  ...
  try {
    ...
    if (tomlData.mcp_servers) {
      // Define known fields that TKP directly manages
      ...
      for (const [id, mcpData] of Object.entries(tomlData.mcp_servers)) {
        ...
        // Collect extra fields not directly managed by TKP
    ...
    if (commentedMatch) {
      ...
      for (const line of lines) {
        ...
        // Skip comments (but reset inSection flag for TKP comments)
        if (trimmedLine.startsWith('#')) {
          if (trimmedLine.includes('--- model provider added by TKP ---')) {
            inSection = false // TKP comments mark global config area
          }
          continue
        }
    ...
    // Preserve other configuration (line-scan, skip TKP-managed sections/fields)
    ...
    for (const line of lines) {
      ...
      // Skip TKP managed comments/headers
      if (/^#\s*---\s*model provider added by TKP\s*---\s*$/i.test(trimmed))
        continue
      if (/^#\s*---\s*MCP servers added by TKP\s*---\s*$/i.test(trimmed))
        continue
      if (/Managed by TKP/i.test(trimmed))
        continue
      ...
      if (sec) {
        ...
        // Do not push the section header if it's TKP-managed; allow non-managed sections to pass
    ...
    const cleaned = content
      // Remove TKP headers
      .replace(/^\s*#\s*---\s*model provider added by TKP\s*---\s*$/gim, '')
      .replace(/^\s*#\s*---\s*MCP servers added by TKP\s*---\s*$/gim, '')
...
export function renderCodexConfig(data: CodexConfigData): string {
  ...
  // CRITICAL: Add TKP global configuration FIRST to ensure it's truly global
  ...
  if (data.model || data.modelProvider || data.providers.length > 0 || data.modelProviderCommented) {
    lines.push('# --- model provider added by TKP ---')
  ...
  // Add preserved non-TKP configuration after global config
  if (data.otherConfig && data.otherConfig.length > 0) {
    const preserved = data.otherConfig.filter((raw) => {
      const l = String(raw).trim()
      if (!l)
        return false
      // Guard: never re-insert any TKP-managed headers/sections/fields
      if (/^#\s*---\s*model provider added by TKP\s*---\s*$/i.test(l))
        return false
      if (/^#\s*---\s*MCP servers added by TKP\s*---\s*$/i.test(l))
        return false
  ...
  if (data.mcpServices.length > 0) {
    lines.push('')
    lines.push('# --- MCP servers added by TKP ---')
...
export async function runCodexSystemPromptSelection(skipPrompt = false): Promise<void> {
  ...
  const zcfConfig = readTkpConfig()
  const { readDefaultTomlConfig } = await import('../tkp-config')
  const tomlConfig = readDefaultTomlConfig()
  ...
  updateTkpConfig({ templateLang: preferredLang })
  ...
  // Update TKP configuration to save the selected system prompt style
  try {
    const { updateTomlConfig } = await import('../tkp-config')
    const { TKP_CONFIG_FILE } = await import('../../constants')
    ...
    console.error('Failed to update TKP config:', error)
  }
}
