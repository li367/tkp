---
description: Manage Git worktrees in project-level ../.tkp/project-name/ directory with smart defaults, IDE integration and content migration
...
Manage Git worktrees with smart defaults, IDE integration and content migration in structured `../.tkp/project-name/` paths.
...
| Option             | Description                                            |
| ------------------ | ------------------------------------------------------ |
| `add [<path>]`     | Add new worktree in `../.tkp/project-name/<path>`      |
...
2. **Smart Path Management**
   ...
   - Create worktrees in structured `../.tkp/project-name/<path>` directory
...
WORKTREE_BASE="$MAIN_REPO_PATH/../.tkp/$PROJECT_NAME"

# Always use absolute path to prevent nesting issues
ABSOLUTE_WORKTREE_PATH="$WORKTREE_BASE/<path>"
```

**Critical Fix**: Always use absolute paths when creating worktrees from within existing worktrees to prevent path nesting issues like `../.tkp/project/.tkp/project/path`.
...
6. **Safety Features**
   ...
   - **Absolute path enforcement**: Prevent nested `.tkp` directories when in worktree
...
```
✅ Worktree created at ../.tkp/project-name/feature-ui
✅ Copied .env
✅ Copied .env.local
📋 Copied 2 environment file(s) from .gitignore
🖥️ Open ../.tkp/project-name/feature-ui in IDE? [y/n]: y
🚀 Opening ../.tkp/project-name/feature-ui in VS Code...
```
