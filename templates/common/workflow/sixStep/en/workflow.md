...
- Store plan summary (with context and plan) in project root directory `.tkp/plan/current/task-name.md`
...
- Naming task plan documents during archiving (when moving from `.tkp/plan/current/` to `.tkp/plan/history/`)
...
- Store execution plan in project root directory `.tkp/plan/current/task-name.md`
...
- After task completion, move plan file from `.tkp/plan/current/` to `.tkp/plan/history/` for archiving
