...
`tkp update`（省略形 `tkp u`）は、ワークフローテンプレート、プロンプト、ツールバージョンの確認を更新するために使用されます。これは軽量な更新コマンドで、API 設定やインストール済みの MCP サービスを変更しません。

## 機能概要

`tkp update` コマンドは以下の操作を実行します：
...
| `--code-type, -T` | `-T` | 対象コードツールタイプ | `claude-code`, `codex`, `cc`, `cx` | TKP 設定の現在のツールタイプ |
...
4. **設定を保存**：TKP グローバル設定を更新

## 更新内容

`tkp update` は以下の内容を更新します：

### ワークフローテンプレート

- ✅ 6段階ワークフロー（`/tkp:workflow`）
- ✅ 機能開発ワークフロー（`/tkp:feat`）
...
これらの内容を更新する必要がある場合は、`tkp init` または対応する設定メニューを使用してください。

## バージョン確認

`tkp update` は自動的にツールバージョンを確認します：
...
ワークフローとプロンプトを更新する前に、TKP は自動的にバックアップを作成します：
...
最新のワークフローと改善を取得するために、定期的に `tkp update` を実行することをお勧めします：
...
2. **`docs-only` モードを使用**：`tkp init` で `--config-action docs-only` を使用
...
3. **check-updates コマンドを使用**：`npx tkp check-updates` を実行
...
> 💡 **推奨事項**：
> - 初回使用時、または API/MCP 設定を変更する必要がある場合、`tkp init` を使用
> - ワークフローとテンプレートのみを更新する必要がある場合、`tkp update` を使用

## 関連リソース

- [tkp init](init.md) - 完全初期化コマンド
...
> 💡 **ヒント**：特に新機能がリリースされたり、テンプレートが更新されたりした場合、ワークフローテンプレートとプロンプトを最新の状態に保つために、定期的に `tkp update` を実行することをお勧めします。
