...
TKPは、増分管理、バックアップ戦略、柔軟な設定切り替えをサポートする包括的な設定管理システムを提供します。設定システムの構造とメカニズムを理解することで、開発環境をより適切に管理および維持できます。

## ディレクトリ構造の概要

TKPの設定は以下のディレクトリに分散しています：
...
| `~/.tkp/` | TKP グローバル設定ディレクトリ | `config.toml` |
...
```
~/.tkp/
├── config.toml            # TKP グローバル設定（TOML形式）
...
非対話モード（`--skip-prompt`）では、TKPは自動的にデフォルト戦略を適用します：
...
TKP は、AI モデル選択をきめ細かく制御するための4モデルアーキテクチャを使用しています：
...
2. **設定ファイル**: `~/.tkp/config.toml` の `templateLang`
...
TKPの旧バージョンからアップグレードする場合：

```bash
# TKPは自動的に設定を検出して移行します
npx tkp init
