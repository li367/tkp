...
6段階ワークフローは TKP のコア開発プロセスであり、完全なソフトウェア開発ライフサイクルをカバーします：**リサーチ → 構想 → 計画 → 実行 → 最適化 → レビュー**。
...
**例**：
```
/tkp:workflow ユーザーログイン機能を実装、メールと電話番号ログインをサポート
```
...
> 💡 **ヒント**：Codex は `/prompts:` プレフィックスを使用し、Claude Code は `/tkp:` プレフィックスを使用します。
...
- 技術実装計画
- 開発計画とタイムライン
- 計画ドキュメント（`.tkp/plan/current/タスク名.md` に保存）
...
1. **コマンド入力**：`/tkp:workflow` または `/prompts:workflow` を入力してタスクを説明
...
- **進行中のタスク**：`.tkp/plan/current/` ディレクトリ
- **完了したタスク**：`.tkp/plan/history/` ディレクトリ

> 💡 **ヒント**：`.tkp/` は統一ワークフローディレクトリであり、Claude Code でも Codex でも同じパスを使用します。
...
```
# メインタスク
/tkp:workflow ユーザー管理システムを構築

# サブタスク 1
/tkp:workflow ユーザー登録機能を実装

# サブタスク 2
/tkp:workflow ユーザーログイン機能を実装

# サブタスク 3
/tkp:workflow ユーザー情報管理を実装
```
...
# 2. 6段階ワークフローで詳細を実装
/tkp:workflow コメントの CRUD 操作と権限制御を実装
...
```
/tkp:workflow ユーザーアバターアップロード機能を追加
```
...
```
/tkp:workflow マイクロサービスアーキテクチャのユーザー認証システムを構築、OAuth2 と JWT をサポート
```
...
```
/tkp:workflow ユーザーモジュールをリファクタリング、コード品質と保守性を向上
```
...
ワークフローは統一された `.tkp/plan/` ディレクトリ構造を使用：
...
- ⚠️ **統一ディレクトリ**：Claude Code でも Codex でも `.tkp/plan/` ディレクトリを使用
...
> 💡 **ヒント**：6段階ワークフローは TKP のコア機能であり、ほとんどの開発タスクに適しています。小さなタスクから始めてプロセスに慣れ、その後徐々により複雑なプロジェクトに適用することを推奨します。段階確認メカニズムを適切に使用することで、より良い開発体験が得られます。
