...
TKP は、複数の設定プロファイルを並行管理し、バックアップ/復元を自動化する包括的なメカニズムを提供します。仕事用と個人用、検証用などを素早く切り替えられます。

## 複数設定システム

### 設定の階層

TKP の設定システムは以下の階層に分かれています：

1. **グローバル設定**（`~/.tkp/config.toml`）- TKP 自体の設定
...
TKP はこれらの設定を作成、管理、切り替えるための強力な CLI ツールを提供します。

- **設定作成**：`tkp init` コマンドを使用して初期化時に複数の API プロバイダーを設定できます。
- **設定切替**：`tkp config-switch` コマンドを使用して、異なる環境、プロジェクト、プロバイダー間を素早く切り替えます。
...
TKP は設定変更のたびにタイムスタンプ付きバックアップを自動作成し、設定の安全性と回復可能性を確保します。
...
| **TKP グローバル** | `~/.tkp/backup/` | `config.toml.{timestamp}.bak` |
...
1. **初期化**：初回設定または再初期化
2. **設定更新**：`tkp update` によるワークフローまたはテンプレートの更新
...
2. **復元する**：手動でバックアップファイルを元の場所にコピーするか、`tkp init` のマージ機能を使用する

## 増分管理

既存の設定が検出された場合、TKP は管理戦略の選択を求めます：
