...
本文档收录了 TKP 在日常使用中的实用技巧和最佳实践，帮助你在各种场景中更高效地使用 TKP。

## 核心技巧

### 1. 善用交互式菜单

**技巧**：习惯从 `npx tkp` 开始，所有功能都有编号提示，避免记忆命令细节。
...
if [ "$TOKENS" -gt 1000000 ]; then
  echo "警告：Token 使用量超过 100 万！" | mail -s "TKP 使用量告警" admin@example.com
fi
```
...
```bash
# 1. 仅安装需要的 MCP 服务
npx tkp init -s -m context7,open-websearch  # 仅安装必需服务
...
# 团队成员使用相同配置
npx tkp init -s --api-configs-file team-config.json -k "个人API密钥"
```
