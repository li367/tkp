...
执行 `npx tkp` 会进入交互式菜单系统。菜单是 TKP 的核心交互界面，提供可视化的操作选项，无需记忆复杂的命令参数。
...
| 选项 | 功能 | 对应命令 | 说明 |
|------|------|---------|------|
| `1` | 完整初始化 | `tkp init` | 完整初始化 Claude Code 环境 |
| `2` | 导入/更新工作流 | `tkp update` | 更新工作流模板和提示词 |
...
| `R` | CCR 管理菜单 | `tkp ccr` | Claude Code Router 管理 |
| `U` | Claude Code 使用分析 | `tkp ccu` | 查看 API 使用统计 |
| `L` | CCometixLine 管理 | - | 状态栏工具管理 |
| `0` | 切换脚本语言 | - | 切换 CLI 界面语言（zh-CN/en） |
| `S` | 切换代码工具 | - | 在 Claude Code ↔ Codex 之间切换 |
| `-` | 卸载当前工具配置 | `tkp uninstall` | 卸载 Claude Code 配置 |
| `+` | 检查更新 | `tkp check-updates` | 检查工具版本并更新 |
| `Q` | 退出 | - | 退出菜单 |
...
# 2. 手动切换
npx tkp init -T codex  # 切换到 Codex
npx tkp init -T claude-code  # 切换到 Claude Code
```

## 相关资源

- [tkp init](init.md) - 完整初始化命令详解
- [tkp update](update.md) - 更新命令详解
- [快速开始](../getting-started/installation.md) - 安装和使用指南

> 💡 **提示**：菜单是 TKP 最友好的使用方式，特别适合不熟悉命令行的用户。建议首次使用时通过菜单完成配置，熟悉后可以结合直接命令使用提高效率。