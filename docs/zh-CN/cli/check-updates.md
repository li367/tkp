...
`tkp check-updates` 用于检测并更新 TKP 工具链中的各个组件，包括 TKP 本身、Claude Code、CCR、CCometixLine、Codex 等工具。

> **别名**：可使用同等效果的 `tkp check`（如 `npx tkp check -T cx`）。
...
| 参数 | 简写 | 说明 | 可选值 | 默认值 |
|------|------|------|--------|--------|
| `--code-type, -T` | `-T` | 指定工具类型 | `claude-code`, `codex`, `cc`, `cx` | 从 TKP 配置读取 |
