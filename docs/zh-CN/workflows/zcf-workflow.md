...
六阶段工作流是 TKP 的核心开发流程，涵盖完整的软件开发生命周期：**研究 → 构思 → 计划 → 执行 → 优化 → 评审**。
...
**示例**：
```
/tkp:workflow 实现用户登录功能，支持邮箱和手机号登录
```
...
> 💡 **提示**：Codex 使用 `/prompts:` 前缀，而 Claude Code 使用 `/tkp:` 前缀。
...
- 技术实现方案
- 开发计划和时间线
- 计划文档（保存在 `.tkp/plan/current/任务名.md`）
...
1. **输入命令**：输入 `/tkp:workflow` 或 `/prompts:workflow` 并描述任务
2. **阶段执行**：AI 按顺序执行六个阶段
...
- **进行中的任务**：`.tkp/plan/current/` 目录
- **已完成的任务**：`.tkp/plan/history/` 目录

> 💡 **提示**：`.tkp/` 是统一的工作流目录，无论使用 Claude Code 还是 Codex 都使用相同的路径。
...
```
# 主任务
/tkp:workflow 构建用户管理系统

# 子任务 1
/tkp:workflow 实现用户注册功能

# 子任务 2
/tkp:workflow 实现用户登录功能

# 子任务 3
/tkp:workflow 实现用户信息管理
```
...
# 2. 使用六阶段工作流实现细节
/tkp:workflow 实现评论的 CRUD 操作和权限控制
...
```
/tkp:workflow 构建微服务架构的用户认证系统，支持 OAuth2 和 JWT
```
...
```
/tkp:workflow 重构用户模块，提升代码质量和可维护性
```
...
- ⚠️ **统一目录**：无论使用 Claude Code 还是 Codex，都使用 `.tkp/plan/` 目录
...
> 💡 **提示**：六阶段工作流是 TKP 的核心功能，适合大多数开发任务。建议先从小任务开始熟悉流程，然后逐步应用到更复杂的项目中。合理利用阶段确认机制，可以获得更好的开发体验。