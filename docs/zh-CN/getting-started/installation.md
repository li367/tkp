...
本指南将帮助您快速开始使用 TKP，从环境检查到验证使用的完整流程。无论您是首次使用还是希望在新设备上快速部署，都可以按照本指南完成设置。

> 💡 **提示**：TKP 无需安装，直接使用 `npx tkp` 即可运行。本指南主要介绍环境配置和使用流程。
...
> 💡 **提示**：如果您使用 WSL（Windows Subsystem for Linux），TKP 会自动检测环境并提供相应的安装提示。
>
> 🔐 **Linux 用户**：在 Linux 系统上，TKP 会自动检测您是否以非 root 用户运行，并在需要时使用 `sudo` 进行全局 npm 安装。在安装 Claude Code、Codex、CCR 或 CCometixLine 时，您可能需要输入密码。
...
TKP 提供两种使用方式：**交互式使用**（适合新手）和**命令行直接使用**（适合自动化和 CI/CD）。

> 💡 **提示**：TKP 无需安装，直接使用 `npx tkp` 命令即可运行。

### 方式一：交互式使用（推荐新手）

TKP 提供了友好的交互式菜单，让您可以通过图形化界面完成所有配置。
...
首次运行时，TKP 会显示欢迎界面并询问您希望使用的界面语言：
...
请选择功能:
  ...
  ------------ TKP ------------
  0. 更改显示语言 / Select display language - 更改 TKP 界面语言
...
> ✅ **自动处理**：
> - TKP 会自动检测您的平台并推荐最佳的安装方法
> - 如果 Claude Code 已安装，TKP 会检测版本并可以自动升级到最新版本
> - 安装方法会被保存以供将来参考
...
TKP 支持 API 提供商预设，可将配置从 5+ 个参数简化为仅需 2 个：
...
> 📖 **完整参数列表**：详细参数说明请参考 [CLI 命令 - tkp init](../cli/init.md) 章节。

## Codex 支持

TKP 提供完整的 Codex 支持，让您可以在同一个工具中管理 Claude Code 和 Codex 两种环境。
...
TKP 全面支持跨平台运行，包括 Windows、macOS、Linux、WSL 和 Termux。
...
> ⚠️ **注意**：如果在 Windows 上遇到 MCP 连接问题，运行 `npx tkp` 会自动修复配置格式。
...
> 📱 **提示**：在 Termux 中，TKP 会自动识别特殊的路径结构并正确安装依赖。
...
> 
> 💡 **提示**：Codex 使用 `/prompts:` 前缀，而 Claude Code 使用 `/tkp:` 或直接 `/` 前缀。
...
> 🔧 **故障排除**：如果服务未连接，运行 `npx tkp` → `4` 重新配置 MCP 服务。
...
> 📖 **说明**：`tkp update` 默认会保留您现有的 API 配置和 MCP 设置，只更新工作流模板和文档。
...
如果文件不存在，运行 `npx tkp update` 重新导入。

### Q: MCP 服务连接失败？

**A**: 
1. 检查 MCP 服务配置：`npx tkp` → `4`
...
**A**: 使用配置切换命令：
```bash
npx tkp config-switch --list  # 列出所有配置
npx tkp cs provider-name      # 切换到指定配置
```

### Q: 配置文件保存在哪里？

**A**: 
- **TKP 配置**：`~/.tkp/config.toml`
...
1. 🎯 探索 [功能特性](../features/) 了解 TKP 的完整能力
