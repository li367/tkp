...
本章介绍如何在几分钟内通过 TKP 初始化环境，涵盖安装前准备、常见命令与推荐操作顺序。

## 快速导航

如果你首次接触 TKP，建议按照以下步骤操作：
...
1. 运行 `npx tkp` 或 `npx tkp init` 进行完整初始化
...
- 运行 `npx tkp update` 更新工作流和模板
- 使用 `npx tkp` 打开交互式菜单进行增量配置
- 通过 `npx tkp config-switch` 切换不同的配置
