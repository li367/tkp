...
CCometixLine 是基于 Rust 的高性能终端/IDE 状态栏插件，TKP 支持全自动安装、配置和更新。它可以实时显示 Git 分支信息、文件变更状态、Claude Code / Codex 使用统计等关键信息。
...
> 💡 **提示**：`tkp init` 默认启用 `--install-cometix-line true`，若无需安装可显式传入 `false`。
...
TKP 在初始化或更新时会自动检查 CCometixLine 版本：
...
CCometixLine 可以与以下 TKP 工具无缝集成：

- **ccusage**：共享使用统计数据
- **CCR**：显示代理路由状态（如果配置）
- **工作流**：根据工作流状态显示相应信息

> 💡 **提示**：CCometixLine 是 TKP 生态的重要组成部分，建议在初始化时一起安装，以获得完整的状态监控体验。