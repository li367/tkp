...
TKP 提供完善的配置管理系统，支持增量管理、备份策略和灵活的配置切换。了解配置系统的结构和机制，可以帮助您更好地管理和维护开发环境。
...
在非交互模式下（`--skip-prompt`），TKP 会自动应用默认策略：
...
2. **配置文件**：`~/.tkp/config.toml` 中的 `templateLang`
