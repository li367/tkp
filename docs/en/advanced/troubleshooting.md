...
This document compiles common problems and solutions encountered when using TKP, helping you quickly locate and resolve issues.
...
**Symptoms**: No response or error after running `npx tkp init`
...
mkdir -p ~/.claude ~/.codex ~/.tkp
chmod 755 ~/.claude ~/.codex ~/.tkp

# 3. Skip MCP installation (if network issues)
npx tkp init -s -m skip
...
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.tkp\tkp"
...
# Or use command line
npx tkp init -s -t api_key -k "your-api-key"
...
npx tkp init -s -p 302ai -k "correct-key"
```
...
npx tkp config-switch provider-name
...
cat ~/.claude/workflows/tkp-workflow/workflow.md
```

### 2. Workflow Commands Not Recognized

**Symptoms**: No response when entering `/tkp:workflow`
...
head -20 ~/.claude/workflows/tkp-workflow/workflow.md

# 3. Restart Claude Code or Codex application

# 4. Check command prefix
# Claude Code: /tkp:workflow or /workflow
...
# Or force update
npx tkp init --config-action docs-only -w all
```
...
# 2. Ensure managed = true (TKP-managed configuration)
# If managed = false, TKP will not modify configuration
...
# 5. Reconfigure
npx tkp init -T codex -s -p 302ai -k "sk-xxx"
```
...
npx tkp init --config-action merge
...
```bash
# TKP automatically handles spaces in paths
...
```bash
# TKP automatically fixes Windows MCP configuration format
...
**Symptoms**: TKP did not correctly detect WSL environment

**Solutions**:

```bash
# TKP automatically detects WSL environment
...
```bash
# TKP supports Termux environment
...
npx tkp init --verbose 2>&1 | tee tkp-debug.log

# View error messages
cat tkp-debug.log | grep -i error
...
- Check [GitHub Issues]() to search for similar problems
- When submitting issues on GitHub Issues, include:
  - `npx tkp --version` output
