...
`tkp config-switch` is used to quickly switch between multiple API configurations, suitable for users who use different API providers for different projects.

> **Alias**: `tkp cs` provides the same functionality, so any example can be shortened (for example `npx tkp cs --list`).
...
npx tkp cs provider1

# Specify tool type (Supports short alias -T cc/cx)
npx tkp cs --list -T cc      # List Claude Code configs
npx tkp cs --list -T cx      # List Codex configs
npx tkp cs provider1 -T cx   # Switch Codex config
```
...
|------|------|--------|--------|
| `--code-type`, `-T` | Specify tool type | `claude-code` (cc), `codex` (cx) | Read from TKP configuration |
...
3. **Custom Configuration**: Multiple API configurations created through `tkp init`
...
2. **Custom Providers**: Providers configured through `tkp init` (like 302.AI, GLM, etc.)
...
npx tkp cs --list -T cc

# Codex configurations
npx tkp cs --list -T cx
```
...
npx tkp cs glm-provider

# Codex switch provider
npx tkp cs glm-provider -T cx
```
...
npx tkp cs 302ai-provider

# Project C uses MiniMax
npx tkp cs minimax-provider
```
...
cd ../.tkp/project-name/feat/new-feature
npx tkp cs 302ai-provider
```
...
A: You can manage configurations through the TKP Main Menu:

1. Run `npx tkp` to enter the main menu
