...
`tkp init` (abbreviation `tkp i`) is TKP's core command for complete initialization of Claude Code or Codex environment. It automatically installs necessary tools, configures API, sets up MCP services, imports workflows and output styles, etc.

## Feature Overview

The `tkp init` command performs the following operations:
...
In interactive mode, TKP will guide you step by step:
...
| `--lang, -l` | `-l` | TKP interface display language | `zh-CN`, `en` | User preference or `en` |
...
TKP supports API provider presets, which can greatly simplify configuration:
...
npx tkp i -s --config-action backup  # Backup then overwrite (default)
npx tkp i -s --config-action merge   # Merge configuration
npx tkp i -s --config-action new     # Create new configuration
npx tkp i -s --config-action docs-only  # Only update documents
npx tkp i -s --config-action skip    # Skip configuration

# Control CCometixLine installation
npx tkp i -s -x true   # Install (default)
npx tkp i -s -x false  # Don't install
```
...
When existing configuration is detected, TKP will ask for handling strategy:
...
The execution flow of `tkp init` is as follows:

1. **Display Banner**: Display TKP version information and tool type
...
11. **Save Preferences**: Update TKP global configuration
