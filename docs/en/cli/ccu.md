...
`tkp ccu` (Claude Code Usage) is used to view and analyze Claude Code usage statistics, helping you understand AI assistant usage and costs.
...
npx tkp ccu --period daily
npx tkp ccu --period weekly
npx tkp ccu --period monthly
...
```bash
npx tkp ccu --json --period weekly
```
...
```bash
npx tkp ccu --csv --period monthly > usage.csv
```
...
npx tkp ccu --period weekly --json > weekly-usage.json
```
...
npx tkp ccu --period monthly --csv > monthly-usage.csv
...
0 23 * * * cd /path/to/project && npx tkp ccu --json --period daily >> usage.log
```
...
1. Install CCometixLine: `npx tkp` → Select corresponding option
