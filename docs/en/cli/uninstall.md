...
`tkp uninstall` provides a safe uninstallation process, supporting selective uninstall, complete uninstall, and conflict resolution. Suitable for scenarios requiring environment reset, device migration, or configuration cleanup.

## Feature Overview

The `tkp uninstall` command supports:

1. 🗑️ **Selective Uninstall**: Selectively delete specific components
2. 🔄 **Complete Uninstall**: Completely remove all TKP configurations and tools
...
In interactive mode, TKP will guide you:
...
Completely remove all TKP-related configurations and tools:
...
npx tkp uninstall --mode complete

# Specify language
npx tkp uninstall --mode complete --lang zh-CN
```
...
npx tkp uninstall --mode custom --items "ccr,backups,cometix"

# Use array format (in code)
npx tkp uninstall --mode custom --items '["ccr","backups"]'
```
...
Remove all TKP-related configurations and tools:
...
| `tkp-config` | TKP global configuration | `~/.tkp/` |

```bash
# Only uninstall CCR
npx tkp uninstall --mode custom --items ccr

# Uninstall multiple components
npx tkp uninstall --mode custom --items "ccr,cometix,backups"
...
npx tkp uninstall --mode custom --items cometix
```
...
Before executing uninstall, TKP will:
...
# Backup TKP configuration
tar -czf tkp-backup.tar.gz ~/.tkp/
```
...
npx tkp uninstall --mode complete
npx tkp init -s -p 302ai -k "test-key" -g zh-CN
```
...
| Operation | `tkp uninstall` | `tkp init --config-action new` |
|------|----------------|-------------------------------|
...
> 💡 **Recommendations**:
> - Use `tkp uninstall` when you need to completely clean environment
> - Use `tkp init --config-action new` when you need to reset configuration but preserve tools

## Related Resources

- [tkp init](init.md) - Reinitialize environment
...
> ⚠️ **Warning**: Uninstall operations are irreversible. Please ensure important configurations are backed up before execution. If you only need to reset part of the configuration, it's recommended to use `tkp init`'s `--config-action` option rather than complete uninstall.
