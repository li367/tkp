...
`tkp ccr` provides a complete management menu for Claude Code Router (CCR), including installation, configuration, service control, and Web UI access.
...
Running `tkp ccr` will display the following menu:
...
**API Key**:
- When starting UI, CCR API key will be displayed (default: `sk-tkp-x-ccr`)
...
| `APIKEY` | string | CCR API key | `sk-tkp-x-ccr` |
...
TKP supports multiple CCR provider presets to simplify configuration:
...
3. Use API key `sk-tkp-x-ccr` to log in (or check `APIKEY` in configuration)
