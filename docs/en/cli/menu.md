...
Running `npx tkp` enters the interactive menu system. The menu is TKP's core interactive interface, providing visual operation options without needing to remember complex command parameters.
...
|------|------|---------|------|
| `1` | Complete Initialization | `tkp init` | Complete Claude Code environment initialization |
| `2` | Import/Update Workflows | `tkp update` | Update workflow templates and prompts |
...
| `R` | CCR Management Menu | `tkp ccr` | Claude Code Router management |
| `U` | Claude Code Usage Analysis | `tkp ccu` | View API usage statistics |
...
| `-` | Uninstall Current Tool Configuration | `tkp uninstall` | Uninstall Claude Code configuration |
| `+` | Check Updates | `tkp check-updates` | Check tool versions and update |
| `Q` | Exit | - | Exit menu |
...
Equivalent to running `npx tkp init`, will guide you through:
...
Equivalent to running `npx tkp update`, will:
...
When using TKP for the first time, it's recommended to start from the menu:
...
# 2. Manually switch
npx tkp init -T codex  # Switch to Codex
npx tkp init -T claude-code  # Switch to Claude Code
```

## Related Resources

- [tkp init](init.md) - Detailed complete initialization command
- [tkp update](update.md) - Detailed update command
- [Quick Start](../getting-started/installation.md) - Installation and usage guide

> 💡 **Tip**: The menu is TKP's most user-friendly way to use, especially suitable for users unfamiliar with command line. It's recommended to complete configuration through the menu on first use, and after familiarizing yourself, you can combine direct commands to improve efficiency.
