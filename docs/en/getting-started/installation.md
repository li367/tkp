...
This guide will help you quickly get started with TKP, covering the complete process from environment check to verification. Whether you're using it for the first time or want to quickly deploy on a new device, you can complete the setup by following this guide.

> 💡 **Tip**: TKP requires no installation, just run `npx tkp` directly. This guide mainly covers environment configuration and usage flow.
...
> 💡 **Tip**: If you use WSL (Windows Subsystem for Linux), TKP will automatically detect the environment and provide corresponding installation prompts.
>
> 🔐 **Linux Users**: On Linux systems, TKP automatically detects if you're running as a non-root user and will use `sudo` for global npm installations when needed. You may be prompted for your password during installation of Claude Code, Codex, CCR, or CCometixLine.
...
TKP provides two usage methods: **Interactive Use** (suitable for beginners) and **Command Line Direct Use** (suitable for automation and CI/CD).

> 💡 **Tip**: TKP requires no installation, just use the `npx tkp` command to run.

### Method 1: Interactive Use (Recommended for Beginners)

TKP provides a friendly interactive menu that allows you to complete all configurations through a graphical interface.
...
On first run, TKP will display a welcome screen and ask you which interface language you want to use:
...
Please select function:
  ...
  ------------ TKP ------------
  0. Change Display Language / Select display language - Change TKP interface language
...
When selecting `1` to perform complete initialization, TKP will guide you through the following steps:
...
If you choose to install, TKP will prompt you to select an installation method:
...
> - TKP automatically detects your platform and recommends the best installation method
> - If Claude Code is already installed, TKP will detect the version and can automatically upgrade to the latest version
...
TKP supports API provider presets, which can simplify configuration from 5+ parameters to just 2:
...
> 📖 **Complete Parameter List**: For detailed parameter descriptions, please refer to the [CLI Commands - tkp init](../cli/init.md) chapter.

## Codex Support

TKP provides complete Codex support, allowing you to manage both Claude Code and Codex environments in the same tool.
...
npx tkp → Select S (Switch Tool) → Select 1 (Complete Initialization)
```
...
TKP fully supports cross-platform operation, including Windows, macOS, Linux, WSL, and Termux.
...
> ⚠️ **Note**: If you encounter MCP connection issues on Windows, running `npx tkp` will automatically fix the configuration format.
...
> 📱 **Tip**: In Termux, TKP will automatically recognize special path structures and correctly install dependencies.
...
```bash
# Check if TKP command is available
npx tkp --help
...
**Claude Code:**
```
/tkp:workflow  # Six-stage development workflow
/tkp:feat      # Feature development workflow
...
> 
> 💡 **Tip**: Codex uses `/prompts:` prefix, while Claude Code uses `/tkp:` or direct `/` prefix.
...
> 🔧 **Troubleshooting**: If services are not connected, run `npx tkp` → `4` to reconfigure MCP services.
...
> 📖 **Note**: `tkp update` will preserve your existing API configuration and MCP settings by default, only updating workflow templates and documents.
...
If files don't exist, run `npx tkp update` to re-import.

### Q: MCP service connection failed?

**A**: 
1. Check MCP service configuration: `npx tkp` → `4`
...
```bash
npx tkp config-switch --list  # List all configurations
npx tkp cs provider-name      # Switch to specified configuration
```

### Q: Where are configuration files saved?

**A**: 
- **TKP Configuration**: `~/.tkp/config.toml`
...
1. 🎯 Explore [Features](../features/) to understand TKP's complete capabilities
